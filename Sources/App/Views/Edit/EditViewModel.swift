...
class EditViewModel: BaseViewModel {
    ...
    let defaultError = NSError(domain: "", code: 12390, userInfo: [NSLocalizedDescriptionKey: "Something went wrong. Please try again!"])
    ...
    var brushConfigValue: Int {
        remoteConfigManager.brushSizeConfigValue
    }
    ...
    var genEditWhenRwDismissCreateOsb: Observable<Bool> {
        modelManager.genEditWhenRwDismissCreateOsb
    }
    
    var genEditWhenRwDismissCreateValue: Bool {
        modelManager.genEditWhenRwDismissCreateValue
    }
...
extension EditViewModel {
    //MARK: Reward ads
    func updateEditGenWhenRwCreateDismiss(isGen: Bool) {
        modelManager.updateGenEditWhenRwCreateDismiss(isGen: isGen)
    }
    ...
    func updateUsage() {
        if userDefault.isPurchase {
            userDefault.usage.editUsagePremiumCount += 1
        }
        // Update usage left
        modelManager.updateEditUsageLeft()
    }
            ...
            if response.status == "succeeded" {
                ...
                return Observable<Int>.timer(.seconds(1), scheduler: scheduler.main)
                    .flatMap { _ in
                        ...
                        self.pollGetReplicateResultStringOutput(url: url)
                    }
            } else {
    ...
    func requestFluxFillDev(userInput: String, controlImage: String, markImage: String) -> Observable<String> {
        Observable<String>.create { [weak self] observer in
            ...
            APIClient.requestUpdateImageFlux(userInput: userInput, controlImage: controlImage, markImage: markImage) { result, error  in
                if let error = error {
